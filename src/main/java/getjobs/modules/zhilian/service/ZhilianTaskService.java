...
public class ZhilianTaskService {
    ...
    public CollectResult collectJobs(ConfigDTO config) {
        ...
        try {
            ...
            log.info("智联招聘岗位采集操作完成，采集到 {} 个岗位，保存到数据库 {} 个", allJobDTOS.size(), savedCount);
            return result;

        } catch (Exception e) {
            log.error("智联招聘岗位采集操作执行失败", e);
    ...
    public DeliveryResult deliverJobs(ConfigDTO config, boolean enableActualDelivery) {
        ...
        try {
            ...
            log.info("智联招聘岗位投递操作完成，处理 {} 个岗位", deliveredCount);
            return result;

        } catch (Exception e) {
            log.error("智联招聘岗位投递操作执行失败", e);
    ...
    private void publishTaskUpdate(TaskStage stage, TaskStatus status, Integer count, String message) {
        TaskUpdatePayload payload = TaskUpdatePayload.builder()
                .platform(RecruitmentPlatformEnum.ZHILIAN_ZHAOPIN)
                .stage(stage)
                .status(status)
                .count(count)
                .message(message)
