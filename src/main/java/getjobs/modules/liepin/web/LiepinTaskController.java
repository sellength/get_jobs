...
public class LiepinTaskController {
    ...
    public ResponseEntity<Map<String, Object>> login(@RequestBody ConfigDTO config) {
        ...
        try {
            ...
            if (result.isSuccess()) {
                log.info("猎聘登录成功，任务ID: {}", result.getTaskId());
                return ResponseEntity.ok(response);
            } else {
                log.warn("猎聘登录失败，任务ID: {}, 原因: {}", result.getTaskId(), result.getMessage());
                return ResponseEntity.badRequest().body(response);
            }

        } catch (Exception e) {
            log.error("猎聘登录接口执行异常", e);
            Map<String, Object> response = new HashMap<>();
    ...
    public ResponseEntity<Map<String, Object>> collectJobs(@RequestBody ConfigDTO config) {
        ...
        try {
            ...
            log.info("猎聘岗位采集完成，任务ID: {}, 采集到 {} 个岗位", result.getTaskId(), result.getJobCount());
            return ResponseEntity.ok(response);

        } catch (Exception e) {
            log.error("猎聘岗位采集接口执行异常", e);
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
    ...
    public ResponseEntity<Map<String, Object>> filterJobs(@RequestBody FilterRequest request) {
        ...
        try {
            ...
            log.info("猎聘岗位过滤完成，原始 {} 个，过滤后 {} 个",
                    result.getOriginalCount(), result.getFilteredCount());
            return ResponseEntity.ok(response);

        } catch (Exception e) {
            log.error("猎聘岗位过滤接口执行异常", e);
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
    ...
    public ResponseEntity<Map<String, Object>> deliverJobs(@RequestBody DeliveryRequest request) {
        ...
        try {
            ...
            log.info("猎聘岗位投递完成，任务ID: {}, 处理 {} 个岗位",
                    result.getTaskId(), result.getDeliveredCount());
            return ResponseEntity.ok(response);

        } catch (Exception e) {
            log.error("猎聘岗位投递接口执行异常", e);
            Map<String, Object> response = new HashMap<>();
            response.put("success", false);
    ...
    /**
     * 查询任务状态接口
     * GET /api/liepin/task/status/{taskId}
     *
     * @param taskId 任务ID
     * @return 任务状态
     */
